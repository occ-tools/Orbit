...
export class CredentialsManager {
  ...
  private encryptWindows(plainText: string): string {
    try {
      const cmd =
        'powershell.exe -NoProfile -NonInteractive -Command "$plain = [Console]::In.ReadLine(); if ($plain) { $plain | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString }"';
  ...
  private decryptWindows(cipherText: string): string {
    try {
      const cmd =
        'powershell.exe -NoProfile -NonInteractive -Command "$cipher = [Console]::In.ReadLine(); if ($cipher) { $secure = ConvertTo-SecureString $cipher; [System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($secure)) }"';
      const stdout = execSync(cmd, {
        input: cipherText + "\n",
        encoding: "utf8",
        stdio: ["pipe", "pipe", "ignore"],
